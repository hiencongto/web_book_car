    ...
    <div class="page-body">
        <div class="title-header title-header-1">
            ...
            <form class="d-inline-flex">
                <a href="{{route('show_trio_process')}}" class="align-items-center btn btn-theme">
                    <i data-feather="plus-square"></i>Trip In Processing
                </a>
            </form>
        </div>
                                        ...
                                        @foreach($trips as $trip)
                                            <tr>
                                                ...
                                                <td>
                                                    <ul>
                                                        <li>
                                                            <a href="{{route('show_trip_detail_driver', ['id' => $trip->id])}}">
                                                                <span class="lnr lnr-eye"></span>
                                                            </a>
                                                            @if(! $trips->contains('status_id', CommonConstant::STATUS_ID['trip_on_trip']) &&
                                                                $trip->status_id != CommonConstant::STATUS_ID['trip_finished']
                                                                && ($trip->trip_details->contains('status_id', CommonConstant::STATUS_ID['trip_detail_pending']))
                                                                && (date("Y-m-d")  ==  date("Y-m-d", $trip->time_start))
                                                               )
                                                            <a href="{{route('start_trip',['id' => $trip->id])}}" >
    ...
    <script>
        function filter() {
            ...
            if(from_id || to_id || status){
                window.location = `{{route('filter_trip_driver')}}?from_id=${from_id}&to_id=${to_id}&status=${status}`
            }
            else window.location = '{{route('list_trip_driver')}}'
        }
    </script>
@endsection
